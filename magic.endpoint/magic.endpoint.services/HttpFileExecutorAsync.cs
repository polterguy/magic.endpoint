...
namespace magic.endpoint.services
{
    ...
    public class HttpFileExecutorAsync : IHttpExecutorAsync
    {
         ...
         */
        async Task<Node> ApplyInterceptors(Node result, string hlFile, string htmlFile)
        {
            ...
            while (true)
            {
                ...
                if (_fileService.Exists(current))
                    result = await ApplyInterceptor(result, current, htmlFile);
         ...
         */
        async Task<Node> ApplyInterceptor(Node lambda, string interceptorFile, string html)
        {
